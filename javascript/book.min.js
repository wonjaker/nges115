...
/[\s\r\n]/;var h=g(b.toLowerCase());c=g(c.toLowerCase());c=h.indexOf(c);if(-1!==c){g=[];for(h=0;h<b.length;h++)d.test(b[h])||g.push(h);c<g.length&&(f=g[c])}}else f=b.toLowerCase().indexOf(c.toLowerCase());if(-1===f)return 100<b.length?b.substring(0,100)+"..":b;50<=f?(d=Math.max(50<=e-f?f-50:e-100,0),d=".."+b.substring(d,f)):d=b.substring(0,f);b=50<e-f?b.substring(f,50<=f?f+50:100)+"..":b.substring(f,e);return d+b},onWindowTap=function(b){b=BookInfo.getBook&&BookInfo.getBook();b&&(b.status_zoom||b.isZoomming||
...
401))},stopEvent=function(b){try{var c=b||window.event;c.preventDefault?(c.preventDefault(),c.stopPropagation()):(c.returnValue=!1,c.cancelBubble=!0)}catch(d){}return!1},getPagesByRange=function(b){function c(k){for(var l=0;l<d.length;l++)if(d[l]==k)return!0;return!1}var d=[];if(!b||"string"!==typeof b||""==b.trim())return d;b=b.split(",");for(var e=0;e<b.length;e++){var f=b[e];if(!isNaN(f))f=getRealPageNumber(f),!c(f)&&f<=bookConfig.totalPageCount&&d.push(f);else if(-1<f.indexOf("-")){var g=f.split("-");
if(!(2>g.length)){f=getRealPageNumber(g[0]);var h=getRealPageNumber(g[1]);g=Math.max(f,h);for(f=Math.min(f,h);f<=g;f++)f>bookConfig.totalPageCount||c(f)||d.push(f)}}}return d},isSlideBook=function(){return"slide"==bookConfig.FlipStyle.toLowerCase()||isBelowIE9()?!0:!1};
...
getConfigValue("BindingType","side"));tmpBookConfig.addPaperCoil="loose"==tmpBookConfig.BindingType||"loose2"==tmpBookConfig.BindingType;tmpBookConfig.ToolbarViewMode=getConfigValue("ToolbarViewMode","standard");tmpBookConfig.ToolbarViewMode=tmpBookConfig.ToolbarViewMode.toLocaleLowerCase();tmpBookConfig.isTheBookOpen=getConfigValue("isTheBookOpen",!1);tmpBookConfig.FlipDirection=getConfigValue("FlipDirection",0);tmpBookConfig.FlipDirectiononMobile=getConfigValue("FlipDirectiononMobile",void 0);void 0!==
tmpBookConfig.FlipDirectiononMobile&&(e=Number(tmpBookConfig.FlipDirectiononMobile),tmpBookConfig.FlipDirectiononMobile=0===e||1===e?e:void 0);(isPhone()||isPad())&&void 0!==tmpBookConfig.FlipDirectiononMobile&&(tmpBookConfig.FlipDirection=tmpBookConfig.FlipDirectiononMobile);tmpBookConfig.continuousPageArrangement=getConfigValue("continuousPageArrangement",!0);tmpBookConfig.startingFromTheSide=getConfigValue("startingFromTheSide",!1);tmpBookConfig.automaticAdsorption=getConfigValue("automaticAdsorption",
...
window.ignoreModuleArray.bookConfig)for(f=0;f<window.ignoreModuleArray.bookConfig.length;f++)g=window.ignoreModuleArray.bookConfig[f],void 0!=tmpBookConfig[g]&&(tmpBookConfig[g]=!1)}catch(m){}checkFlipStyleConfig();return"issuu"==tmpBookConfig.FlipStyle?l.yield(loadCSS(tmpBookConfig.UIBaseURL+"style/issuubook.min.css"),7):l.yield(loadCSS(tmpBookConfig.UIBaseURL+"style/flipbook.min.css"),7);case 7:return checkPageEditorConfig(),recordError(),h=Module._getVerifyString(),k=UTF8ToString(h),Module._free(h),
l.yield(runDecryptedScript(k),0)}})};function recordError(){try{var b=window.setInterval(function(){window.onBookError&&(tmpBookConfig.largePageWidth&&tmpBookConfig.largePageHeight&&tmpBookConfig.totalPageCount||BookEvent.trigger("errorLog",[JSON.stringify({largePageWidth:tmpBookConfig.largePageWidth,largePageHeight:tmpBookConfig.largePageHeight,totalPageCount:tmpBookConfig.totalPageCount,type:"BookConfigError"})]),window.clearInterval(b))},500)}catch(c){}}
...
tmpBookConfig.bottomMargin;try{t&&BookEvent&&BookEvent.trigger&&BookEvent.trigger("layoutMarginChanged",[{topMargin:tmpBookConfig.topMargin,bottomMargin:tmpBookConfig.bottomMargin,reason:e.pendingReason||f.reason||""}])}catch(r){}f.disableResize||e.pendingDisableResize||!t||"function"!=typeof global.onStageResize||onStageResize()}catch(r){console.log("LayoutMarginManager.computeAndApply error",r)}}if(!global.LayoutMarginManager){var e={contributions:{},lastApplied:{topMargin:void 0,bottomMargin:void 0},
...
var VERSION_NAME="7.0.0",BUILD_DATE="2026060901",BookType={normal_book:0,single_book:1,slide_book:3,singleSlide_book:6,catalog_book:7,singleCatalog_book:8,documentView_book:9,singleDocumentView_book:10,card_book:11,invert_book:12},normalBookStatu={book_statu_ready:0,book_statu_auto_flip:1,book_statu_mouse_flip:2,book_statu_mag_flip:3},singleBookStatu={book_statu_ready:0,book_statu_auto_flip:1,book_statu_mouse_flip:2,book_statu_mag_flip:3},bookCorner={top_left:0,top_right:1,bottom_left:2,bottom_right:3},
...
function parsePageIndexFromLocationHash(b){if(!shouldReadPageIndexFromUrlHash())return b;var c=window.location.hash;return""!=c&&3<c.length&&"#p="==c.substr(0,3)?parseInt(getRealPageNumber(decodeURIComponent(c.substr(3,c.length-3)))):b}
