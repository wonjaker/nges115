...
window.addEventListener("popstate",function(a){try{try{var b=window.location.href,c=b.lastIndexOf("/"),d=b.substr(c+1),e=parseInt(d.removeStartFrom("."));if(isNaN(e)){window.phoneArticleReader&&window.phoneArticleReader.hide();var f=window.location.hash;shouldReadPageIndexFromUrlHash()&&""!=f&&3<f.length&&"#p="==f.substr(0,3)&&(currentPageIndex=parseInt(getRealPageNumber(decodeURIComponent(f.substr(3,f.length-3)))),currentPageIndex=Number.between(currentPageIndex,1,originTotalPageCount),BookInfo.getCurrentPages().includes(currentPageIndex)||
...
function scrapURL(){var a=window.location.hash;shouldReadPageIndexFromUrlHash()&&""!=a&&3<a.length&&"#p="==a.substr(0,3)?("undefined"!=typeof window.hashGotoPageTimer&&clearTimeout(window.hashGotoPageTimer),window.hashGotoPageTimer=window.setTimeout(function(){var b=parseInt(getRealPageNumber(decodeURIComponent(a.substr(3,a.length-3))));GetQueryString("search")||gotoPageFun(b,!1,"visit");TurnonAutoFlip();BookEvent.trigger("visit",[b,_agent_.browser.device,_agent_.system.name,window.screen.width,window.screen.height,
global.templeteName]);window.hashGotoPageTimer&&clearTimeout(window.hashGotoPageTimer)},1E3)):(gotoPageFun(bookConfig.OriginPageIndex,!1,"visit"),BookEvent.trigger("visit",[bookConfig.OriginPageIndex,_agent_.browser.device,_agent_.system.name,window.screen.width,window.screen.height,global.templeteName]),TurnonAutoFlip());0<=window.location.search.indexOf("fullscreen=true")&&function(){toolBar&&toolBar.btnFullscreen&&toolBar.btnFullscreen.trigger(_event._end);$(document).trigger(fullScreenApi.fullScreenEventName)}.delay(this,
50)}
