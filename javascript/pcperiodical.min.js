...
var PcArticlePeriodical=Class({create:function(g){this._super(g,!0);this.fillContent()},onCreate:function(){this._super();this.stage.css({"background-color":"#ffffff",color:"#333940"})},initHtml:function(){this.loading=$("<img alt='' src='"+uiBaseURL+"loading.gif'/>");this.calendarFormUrl=this.getImageUrl("calendar_form");this.cancleUrl=this.getImageUrl("closeForm");this.calendarUrl=this.getImageUrl("calendar");this.articleEmptyUrl=this.getImageUrl("articleEmpty");this.calendarSearchUrl=this.getImageUrl("calendarSearch");
this.calendarCancleUrl=this.getImageUrl("calendarCancle");this.head=$("<div class='head'></div>");this.title2=$("<div class='title2'><img alt='' src='"+this.calendarFormUrl+"'/><span>"+getLanguage("btnCalendar","\u5f80\u671f")+"</span></div>");this.close=$("<div class='close'><img alt='' width='auto' height='auto' src='"+this.cancleUrl+"'/></div>");this.head.append(this.title2);this.head.append(this.close);this.search=$("<div class='search'></div>");this.searchBar=$("<div class='calendar_searchBar'></div>");
this.searchInput=$("<input aria-label='"+getLanguage("ariaSearch","search")+"' class='searchInput' placeholder='\u641c\u7d22\u8fc7\u520a\u6807\u9898'/>");this.searchButton=$("<div class='searchButton'><img alt='' src='"+this.calendarSearchUrl+"'/></div>");this.searchCancleButton=$("<div class='searchCancleButton'><img alt='' src='"+this.calendarCancleUrl+"'/></div>");this.dateButton=$("<div class='dateButton'><input type='text' aria-label='"+getLanguage("ariaDatePicker","date picker")+"' placeholder='\u9009\u62e9\u65e5\u671f' oninput='OnInput(event)' class='jeinput' id='test01'/><img alt='' src='"+
this.calendarUrl+"'/></div>");this.searchInput.on("input propertychange change",function(){var g=this.searchInput.val(),n=this.refreshListByKeywords(g);this.refreshList(n);g?this.searchCancleButton.show():this.searchCancleButton.hide()}.bind(this));this.searchCancleButton.click(function(){this.searchInput.val("");this.searchCancleButton.hide();var g=this.refreshListByKeywords("");this.refreshList(g)}.bind(this));$(this.dateButton.find("#test01")[0]).on("input propertychange change",function(){var g=
$("#test01").val();if(g)g=this.refreshListByDate(g),this.refreshList(g);else this.onRefresh()}.bind(this));this.searchBar.append(this.searchInput);this.searchBar.append(this.searchButton);this.searchBar.append(this.searchCancleButton);this.search.append(this.searchBar);this.search.append(this.dateButton);this.empty=$("<div class='calendar_empty'><img alt='' src='"+this.articleEmptyUrl+"'/><p>\u6ca1\u6709\u76f8\u5173\u7ed3\u679c</p></div>");this.container=$("<div class='container'></div>");this.head.append(this.search);
this.stage.append(this.head);this.stage.append(this.empty);this.stage.append(this.container);$(this.close).changeButtonColor("#333940")},initDatePicker:function(){var g="YYYY-MM-DD";switch(this.data.dateType){case "day":g="YYYY-MM-DD";break;case "week":case "half-month":case "month":g="YYYY-MM";break;case "two-months":case "season":case "half-year":case "year":g="YYYY"}var n=this.data.list&&Array.isArray(this.data.list)&&this.data.list.filter(function(b){return b.time}).map(function(b){return Number(b.time)});
n=0<n.length?Math.min.apply(null,n):0;n=0<n?(new Date(n)).getFullYear().toString()+"-01-01":"";$(this.dateButton.find("#test01")[0]).jeDate({format:g,minDate:n})},refreshListByDate:function(g){for(var n=this.data.list,b=[],G=0;G<n.length;G++)if(n[G].time){var f=dateFormat(n[G].time,"yyyy-MM-dd");f&&0<=f.indexOf(g)&&b.push(n[G])}return b},refreshList:function(g){this.container.empty();if(g&&0!=g.length){this.empty.hide();for(var n={$jscomp$loop$prop$i$1:0};n.$jscomp$loop$prop$i$1<g.length;n={$jscomp$loop$prop$i$1:n.$jscomp$loop$prop$i$1},
n.$jscomp$loop$prop$i$1++){var b=g[n.$jscomp$loop$prop$i$1].time?dateFormat(g[n.$jscomp$loop$prop$i$1].time,"yyyy-MM-dd"):"",G=$("<div class='item' bookUrl='"+g[n.$jscomp$loop$prop$i$1].bookUrl+"' domainUrl='"+g[n.$jscomp$loop$prop$i$1].domainUrl+"'></div>"),f=$("<div class='h2'>"+g[n.$jscomp$loop$prop$i$1].title+"</div>");b=$("<div class='time'>"+b+"</div>");$("<div class='tip'>\u6700\u8fd1\u8bfb\u8fc7</div>");var P=$("<div class='image'></div>");P.css("background-image","url("+g[n.$jscomp$loop$prop$i$1].image+
")");G.append(f);G.append(b);G.append(P);var U=this;G.click(function(K){return function(){var T=$(this).attr("bookUrl"),a=$(this).attr("domainUrl");U.showBook(T,a,g[K.$jscomp$loop$prop$i$1].bLink||"")}}(n));this.container.append(G)}n=$("<div class='tips'>\u6682\u65e0\u66f4\u591a</div>");this.container.append(n)}else this.empty.show()},refreshListByKeywords:function(g){var n=this.data.list;if(!g)return n;for(var b=[],G=0;G<n.length;G++)0<=n[G].title.indexOf(g)&&b.push(n[G]);return b},onRefresh:function(){this.refreshList(this.data.list)},
